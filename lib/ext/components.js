define('aura/ext/components', function() {
  ...
  return function(app) {
    ...
    function invokeCallbacks(stage, fnName, context, args) {
      ...
      var dfds      = [];
      app.core.util.each(callbacks, function(cb, i) {
        if (typeof cb === 'function') {
          dfds.push(cb.apply(context, args));
        }
      });
      var ret = app.core.data.all(dfds);
      return ret;
    }
    ...
    Component.load = function(name, opts) {
      ...
      dfd.promise['catch'](function(err) {
    ...
    function parseComponentOptions(el, namespace, opts) {
      ...
      core.util.each(data, function(v, k) {
    ...
    Component.parseList = function(components) {
      ...
      if (_.isArray(components) && ! _.some(components,_.isElement)) {
        ...
        _.each(core.dom.find(selector, components || 'body'), function(element) {

          var ns = "aura";
          if (appNamespace && (core.dom.data(element, appNamespace +'-component') || core.dom.data(element, appNamespace +'-widget'))) {
            ns = appNamespace;
          }
          var options = parseComponentOptions(element, ns);
    ...
    Component.startAll = function(components) {
      ...
      var loadedComponents = core.data.all(list);
    ...
    return {
      ...
      initialize: function(app) {
        ...
        app.sandbox.start = function (list, options) {
          ...
          var c = Component.startAll(list);
          c.then(function (components) {
            core.util.each(components, function (w) {
              w.sandbox._component = w;
              w.sandbox._parent = self;
              children.push(w.sandbox);
            }, function(err){
              throw new Error("Error while starting components", err);
            }).done();
